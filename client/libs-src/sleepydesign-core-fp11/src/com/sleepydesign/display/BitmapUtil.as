package com.sleepydesign.display
{
	...
	public class BitmapUtil
	{
		...
		{
			return new Bitmap(new BitmapData(width, height, alpha < 1.0, color), "auto", true);
		}
		...
		public static function getBitmap(asset:DisplayObject, isTranslate:Boolean = true, rectangle:Rectangle = null):Bitmap
		{
			...
			var bitmap:Bitmap;
			if (!(asset is Bitmap))
				bitmap = new Bitmap(getBitmapData(asset, isTranslate, rectangle), "auto", true);
			else
				bitmap = new Bitmap(Bitmap(asset).bitmapData, "auto", true);
		...
		public static function getBitmapData(asset:DisplayObject, isTranslate:Boolean = true, rectangle:Rectangle = null):BitmapData
		{
			...
			if (asset.width != 0 || asset.height != 0)
			{
				...
				bitmapData.draw(asset, matrix, asset.transform.colorTransform, null, null, true); //, null, rectangle, true);
		...
		{
			return new Bitmap(resizeBitmapData(bitmapData, width, height), "auto", true);
		}

		public static function resizeBitmapData(bitmapData:BitmapData, width:Number, height:Number, transparent:Boolean = true):BitmapData
		{
			const resizedBitmapData:BitmapData = new BitmapData(width, height, transparent);
		...
		{
			return new Bitmap(scaleBitmapData(bitmapData, scale), "auto", true);
		}

		public static function getColorBoundsRect(bitmapData:BitmapData, mask:uint, color:uint, findColor:Boolean = true):Rectangle
		{
			const colorRect:Rectangle = bitmapData.getColorBoundsRect(mask, color, findColor);

			// work around getColorBoundsRect bug
			if ((colorRect.width * colorRect.height == 0) && (bitmapData.getPixel(0, 0) == color))
				colorRect.setTo(0, 0, 1, 1);
