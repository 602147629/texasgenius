package com.sleepydesign.system
{
	...
	public class SystemUtil
	{
		...
		public static function get versions():Array
		{
		...
		public static function addContext(container:DisplayObjectContainer, label:String, eventHandler:Function = null, separatorBefore:Boolean = false, enabled:Boolean = true, visible:Boolean = true):ContextMenuItem
		{
			try
			{
				const _oldItems:Array = container["contextMenu"] ? container["contextMenu"].customItems.concat() : [];
				const _contextMenu:ContextMenu = container["contextMenu"] = new ContextMenu();
				_contextMenu.hideBuiltInItems();

				_contextMenu.customItems = _contextMenu.customItems.concat(_oldItems);

				const item:ContextMenuItem = new ContextMenuItem(label, separatorBefore, enabled, visible);
		...
		// Hide system menu
		public static function hideBuiltInItems(container:DisplayObjectContainer):ContextMenu
		{
			container.contextMenu = new ContextMenu();

			if (container.contextMenu["hideBuiltInItems"] is Function)
				container.contextMenu["hideBuiltInItems"]();
		...
		{
			const src:Array = uri.split(":")
			const protocal:String = src[0];
			const functionString:String = uri.substring(1 + uri.indexOf(":"));
			const functionName:String = functionString.split("(")[0];
			const argumentString:String = functionString.substring(1 + functionString.indexOf("("), functionString.lastIndexOf(")"))
			const argumentArray:Array = argumentString.split(",");
			var argument:Object;
			const _args:Array = [];
