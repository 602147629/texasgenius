package com.exitapplication.module.external;

public class ExternalConst {
	//url
	public final static String ALL_CARD_URL = "http://texasgenius.com/phpsys/action.php";
	
	
	//server send
	public final static String ON_SIT_COMPLETE = "onsitcomplete";
	public final static String ON_SIT_ERROR = "onsiterror";
	public final static String PROVIDE_CONFIG  = "provideconfig";
	
	public final static String START_GAME_TURN = "startgameturn"; 
	public final static String START_USER_TURN = "startuserturn";
	public final static String START_NEW_ROUND_TURN = "startnewroundturn";
	public final static String END_TURN  = "endturn";
	public final static String SEND_TURN_DATA  = "sendturndata";
	
	
	//client send
	public final static String USER_SIT = "usersit"; 
	public final static String USER_DEAL  = "userdeal";
	public final static String DATA  = "data";
	public final static String SEND_GIFT  = "sendgift";
	public final static String SEND_EMOTION  = "sendemotion";
	
	//var
	public final static String SIT_POSITION = "sitposition";
	public final static String PLAYER_STATUS = "playerstatus";
	public final static String FB_UID = "fbuid";
	public final static String USER_ID = "userid";
	public final static String CONFIG_DATA = "configdata";
	public final static String REASON = "reason";
	public final static String VALUE = "value";
	public final static String TURN = "turn";
	public final static String TIME = "time";

	public final static String TO_USER_ID = "touserid";
}
